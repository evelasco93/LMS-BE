...
export interface IDispositionsServiceStackProps extends NestedStackProps {
  lambdaConfig: ILambdaConfig;
  roleName: string;
  logicalIdPrefix: string;
}

export class DispositionsServiceStack extends NestedStack {
  public readonly lambda: NodejsFunction;

  constructor(
    scope: Construct,
    id: string,
    props: IDispositionsServiceStackProps,
  ) {
    ...
    const role = Role.fromRoleName(
      this,
      `${logicalIdPrefix}-DispositionsLambdaRole`,
      roleName,
    );

    this.lambda = new NodejsFunction(
      this,
      `${logicalIdPrefix}-DispositionsFunction`,
      {
        ...
        bundling: {
          ...
          tsconfig: path.join(
            __dirname,
            "../../../handlers/dispositions/tsconfig.build.json",
          ),
