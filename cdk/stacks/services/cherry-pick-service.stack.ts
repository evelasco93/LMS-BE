...
export interface ICherryPickServiceStackProps extends NestedStackProps {
  lambdaConfig: ILambdaConfig;
  roleName: string;
  logicalIdPrefix: string;
}

export class CherryPickServiceStack extends NestedStack {
  public readonly lambda: NodejsFunction;

  constructor(
    scope: Construct,
    id: string,
    props: ICherryPickServiceStackProps,
  ) {
    ...
    const role = Role.fromRoleName(
      this,
      `${logicalIdPrefix}-CherryPickLambdaRole`,
      roleName,
    );

    this.lambda = new NodejsFunction(
      this,
      `${logicalIdPrefix}-CherryPickFunction`,
      {
        ...
        bundling: {
          ...
          tsconfig: path.join(
            __dirname,
            "../../../handlers/cherry-pick/tsconfig.build.json",
          ),
