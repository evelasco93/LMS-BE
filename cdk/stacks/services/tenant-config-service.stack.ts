...
export class TenantConfigServiceStack extends NestedStack {
  ...
  constructor(
    scope: Construct,
    id: string,
    props: ITenantConfigServiceStackProps,
  ) {
    ...
    const role = Role.fromRoleName(
      this,
      `${logicalIdPrefix}-TenantConfigLambdaRole`,
      roleName,
    );

    this.lambda = new NodejsFunction(
      this,
      `${logicalIdPrefix}-TenantConfigFunction`,
      {
        ...
        bundling: {
          ...
          tsconfig: path.join(
            __dirname,
            "../../../handlers/tenant-config/tsconfig.build.json",
          ),
