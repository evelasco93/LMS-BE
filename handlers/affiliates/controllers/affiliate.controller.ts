...
import { CreateAffiliateRequest, UpdateAffiliateRequest } from '../types/affiliate-request.types';
import { AffiliateStatus } from '../enums/affiliate-status.enum';
import { RestApiResponse } from '../types/common.types';

@injectable()
@apiController('/v2/affiliates')
export class AffiliateController extends Controller {
  constructor(@inject('AffiliateService') private readonly affiliateService: AffiliateService) {
    super();
  }

  @POST('/')
  @produces('application/json')
  async createAffiliate(@body payload: CreateAffiliateRequest): Promise<RestApiResponse> {
