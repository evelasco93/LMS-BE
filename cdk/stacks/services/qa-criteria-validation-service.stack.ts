...
export interface IQaCriteriaValidationServiceStackProps extends NestedStackProps {
  lambdaConfig: ILambdaConfig;
  roleName: string;
  logicalIdPrefix: string;
}

export class QaCriteriaValidationServiceStack extends NestedStack {
  public readonly lambda: NodejsFunction;

  constructor(
    scope: Construct,
    id: string,
    props: IQaCriteriaValidationServiceStackProps,
  ) {
    ...
    const role = Role.fromRoleName(
      this,
      `${logicalIdPrefix}-QaCriteriaValidationLambdaRole`,
      roleName,
    );

    this.lambda = new NodejsFunction(
      this,
      `${logicalIdPrefix}-QaCriteriaValidationFunction`,
      {
        ...
        bundling: {
          ...
          tsconfig: path.join(
            __dirname,
            "../../../handlers/qa/modules/criteria-validation/tsconfig.build.json",
          ),
